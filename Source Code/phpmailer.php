...
function sendmail($tomail, $totmailname , $subject, $message)
{
	...
	$loginid 	= "eagriculture@technopulse.cyou";
	$password 	= "dn3SakS-#8RN";
	$smtpserver = "mail.technopulse.cyou";
	$smtpport 	= 465;
	$mailsender = $config['PROJECT_TITLE'];
	$companyname= $config['PROJECT_TITLE'];
	$facebook = $config['SOCIAL_FACEBOOK'];
	$twitter = $config['SOCIAL_TWITTER'];
	$youtube = $config['SOCIAL_YOUTUBE'];
	$linkedin = $config['SOCIAL_LINKEDIN'];
	$companyaddress  = $config['PROJECT_TITLE'];
	$contactno = $config['PHONE'][0];
	$url = $config['WEB_ADDRESS'];
